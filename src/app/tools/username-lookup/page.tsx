...
export default function UsernameLookupPage() {
  ...
  const ResultItem = ({ label, value, isExact }: { label: string, value: string, isExact?: boolean }) => (
    <div className={`flex items-center justify-between gap-4 text-sm py-2.5 px-3 group bg-background/20 rounded-md border transition-all ${isExact ? 'border-primary shadow-sm shadow-primary/20 bg-primary/5' : 'border-border/10 hover:border-primary/30 hover:bg-primary/5'}`}>
      ...
      >
        View Account <ExternalLink className="size-3" />
      </a>
    </div>
  )
  ...
  return (
    <AppLayout>
      <ToolPageWrapper
        ...
        description="Verify digital signatures across global registries. Prioritizing exact literal matches."
        icon={UserSearch}
      >
          ...
          <Card className="border-primary/20 bg-card/40 backdrop-blur-md">
            <form onSubmit={handleSubmit}>
              <CardHeader>
                ...
                <CardDescription>Enter the literal username or name. We check your input exactly first before searching variations.</CardDescription>
              </CardHeader>
                    ...
                    <Collapsible open={isPlatformsOpen} onOpenChange={setIsPlatformsOpen} className="w-full">
                        ...
                        <CollapsibleContent className="space-y-4 pt-4 px-1 pb-2">
                            <div className="flex items-center justify-end gap-2 mb-2">
                                <Button variant="ghost" type="button" size="sm" onClick={handleSelectAll} className="h-7 text-[10px] px-2 uppercase tracking-tighter">Select All</Button>
                                <Button variant="ghost" type="button" size="sm" onClick={handleDeselectAll} className="h-7 text-[10px] px-2 uppercase tracking-tighter">Deselect All</Button>
                            </div>
                    ...
                    <Card className="bg-background/20 border-border/50 p-4 flex flex-col justify-center">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <Label htmlFor="discovery-mode" className="font-headline text-sm font-semibold cursor-pointer">Discovery Mode</Label>
                                <TooltipProvider>
                                    <Tooltip>
                                        ...
                                        <TooltipContent className="max-w-[250px] bg-card border-border text-xs">
                                            Deep Search enabled: Finds variations and hidden profile links via search engines.
                                        </TooltipContent>
                                    </Tooltip>
                                </TooltipProvider>
                            </div>
                        ...
                        <p className="text-[10px] text-muted-foreground mt-1.5 leading-relaxed uppercase tracking-wider">
                            {includeDiscovery ? "Active: Full Reconnaissance" : "Inactive: Precise Match Only"}
                        </p>
                    </Card>
                </div>
              </CardContent>
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              ...
              <div className="text-center space-y-1">
                <p className="font-headline font-bold text-lg text-primary animate-pulse">Verifying Digital Footprint</p>
                <p className="text-xs text-muted-foreground uppercase tracking-[0.2em]">Checking Literal Matches First...</p>
              </div>
            </div>
          )}
          ...
          {results && (
            <Card className="bg-card/30 backdrop-blur-xl border-primary/10 overflow-hidden shadow-2xl">
              <CardHeader className="bg-primary/5 border-b border-primary/10">
                <div className="flex items-center justify-between">
                    <div>
                        <CardTitle className="font-headline text-2xl text-primary">Recon Intelligence Report</CardTitle>
                        <CardDescription className="text-muted-foreground">Target Seed: <span className="text-accent font-mono font-bold">"{username}"</span></CardDescription>
                    </div>
                    <Badge variant="outline" className="bg-primary/10 text-primary border-primary/30 h-8 px-4">
                        {totalMatches} Active Profiles Found
                    </Badge>
                </div>
              </CardHeader>
              <CardContent className="p-0">
                 {totalMatches > 0 || (includeDiscovery && Object.values(results).some(r => r.discovered.length > 0)) ? (
                          ...
                          <AccordionTrigger className="px-6 py-4 font-headline text-lg hover:no-underline hover:bg-primary/5 transition-colors">
                            <div className="flex items-center gap-4">
                              ...
                              <div className="flex gap-2">
                                  {data.exactMatch && <Badge className="bg-primary shadow-sm shadow-primary/50 border-0">LEGIT MATCH</Badge>}
                                  {data.found.length > 0 && <Badge variant="secondary" className="bg-background/50 border-border/10">+{data.found.length} Variants</Badge>}
                              </div>
                            </div>
                          </AccordionTrigger>
                            ...
                            {data.exactMatch && (
                               <div className="space-y-3">
                                  <div className="flex items-center gap-2 mb-2">
                                      ...
                                      <h4 className="text-[10px] font-bold text-primary uppercase tracking-widest px-2">Exact Literal Match</h4>
                                      <div className="h-px bg-primary/40 flex-grow" />
                                  </div>
                            ...
                            {data.found.length > 0 && (
                              <div className="space-y-3">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <h4 className="text-[10px] font-bold text-primary/60 uppercase tracking-widest px-2">Identified Variations</h4>
                                    <div className="h-px bg-primary/20 flex-grow" />
                                </div>
                            ...
                            {data.discovered.length > 0 && includeDiscovery && (
                              <div className="space-y-3">
                                 <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <h4 className="text-[10px] font-bold text-accent uppercase tracking-widest px-2">Recon Discovery</h4>
                                    <div className="h-px bg-accent/20 flex-grow" />
                                 </div>
                  ...
                  <div className="py-20 text-center space-y-3">
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-widest">The target seed did not yield exact matches on selected platforms.</p>
                  </div>
                )}
              </CardContent>
            </Card>
